...
IFS=:
for i in $PATH ; do
	if [ -x "$i/genisoimage" ] ; then
		GENISOIMAGE="$i/genisoimage"
		break
	fi
done
...
if [[ -n "$GENISOIMAGE" && -n "$ROOT_OF_ISO_PATH" ]]; then
	if [ -f "${ROOT_OF_ISO_PATH}/boot/syslinux/isolinux.bin" ]; then
		...
		# genisoimage manpage: http://linux.die.net/man/8/genisoimage
		#
		...
		#
		# Note: UBCD2ISO_ARGS is used to pass user-specific arguments to genisoimage without modifying the script
		"$GENISOIMAGE" -iso-level 4 -l -r -J -joliet-long -D -V "${VOLUME_ID}" \
...
elif [ -z "$GENISOIMAGE" ]; then
	echo "UBCD2ISO ERROR: The 'genisoimage' program could not be found. Install it and try again."
fi
